...
const slice$7 = ArrayProto.slice;
...
function createIndexFinder (dir, predicateFind, sortedIndex) {
	return function (array, item, idx) {
		...
		if (item !== item) {
			idx = predicateFind(slice$7.call(array, i, length), _isNaN);
...
function load(accession) {
    var promise = new Promise(function (resolve) {
        return json('http://www.ebi.ac.uk/proteins/api/proteins/interaction/' + accession + '.json', function (json) {
...
function render(_ref) {
    ...
    select(el).append('div').attr('class', 'interaction-spinner');
...
function draw(el, accession, data) {
    ...
    select(el).select('.interaction-spinner').remove();
    ...
    var x = band().rangeRound([0, width]),
        intensity = linear$2().range([0.2, 1]);
    ...
    function mouseover(p) {
        ...
        selectAll('.interaction-viewer-group').append('line').attr('class', 'active-row').attr('style', 'opacity:0').attr('x1', 0).attr('y1', x(p.source) + x.bandwidth() / 2).attr('x2', x(p.id)).attr('y2', x(p.source) + x.bandwidth() / 2).transition(50).attr('style', 'opacity:.5');

        selectAll('.interaction-viewer-group').append('line').attr('class', 'active-row').attr('style', 'opacity:0').attr('x1', x(p.id) + x.bandwidth() / 2).attr('y1', 0).attr('x2', x(p.id) + x.bandwidth() / 2).attr('y2', x(p.source)).transition(50).attr('style', 'opacity:.5');
    }
...
function hasFilterMatch(source, target, filters) {
    ...
    return _intersection(source.filterTerms, filters.map(function (item) {
        return item['name'];
    })).length > 0 || _intersection(target.filterTerms, filters.map(function (item) {
        return item['name'];
    })).length > 0;
}
...
var loadComponent = function loadComponent() {
    var InteractionViewer = function (_HTMLElement) {
        ...
        function InteractionViewer() {
            ...
            return possibleConstructorReturn(this, (InteractionViewer.__proto__ || Object.getPrototypeOf(InteractionViewer)).apply(this, arguments));
        }
